...
void blackContours(VideoCapture capture, int delay, Rect Get_Area)
{
	...
	while (capture.read(frame))
	{
		...
		detectArea = frame_AfterPreClone(Get_Area);//ROI

		//2.3- 二值化（留下黑色像素）//H低，V低
		threshold(frame_AfterPre, frameBin, 40, 255, THRESH_BINARY_INV);
		frameBinClone = frameBin.clone();
		detectArea_Bin = frameBinClone(Get_Area);
...
void bwboundaries(Mat detectArea, Mat originalFrame , Rect ROI)
{
	...
	*/
	...
	//计算拥挤度等级并在图片上输出拥挤度信息
	crowdHierarchy(peopleCount, countorsArea_Percentage,originalFrame);
