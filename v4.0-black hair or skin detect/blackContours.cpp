...
void blackContours(VideoCapture capture, int delay, Rect Get_Area)
{
	...
	while (capture.read(frame))
	{
		...
		*/
		...
		//cv::Mat detectArea_Bin_inverse = ~detectArea_Bin;
		cv::Mat detectArea_Bin_inverse = detectArea_Bin.clone();

		imshow("hsv图像二值化分割取反", detectArea_Bin_inverse);
	

		// 以下仅对detectArea_Bin进行处理
		//膨胀
		Mat element = getStructuringElement(MORPH_RECT, Size(3, 3));
		dilate(detectArea_Bin_inverse, detectArea_Bin_inverse, element);
		medianBlur(detectArea_Bin_inverse, detectArea_Bin_inverse, 11);

		//所有imshow操作
		bwboundaries(detectArea_Bin_inverse, frame, Get_Area);
